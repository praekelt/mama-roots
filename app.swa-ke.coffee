...
module.exports =
  ...
  locals:
    ...
    current_locale_isocode: 'sw'
    collections: {}
    articles_path: "/lang/#{slugify(current_locale)}/makala"
    category_path: "/lang/#{slugify(current_locale)}/makala/kiwanja"
    slugify: slugify
    moment: moment
    locales: [{
      name: 'English',
  ...
  extensions: [
    records
      ...
      categories:
        ...
        out: (category) ->
          "/makala/kiwanja/#{slugify(category.title)}"
        hook: (categories) ->
          categories = categories.filter filter_current_locale
      pages: 
